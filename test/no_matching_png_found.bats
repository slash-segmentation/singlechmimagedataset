...
# is peachy.
#
@test "Test where no matching png file found" {

  # verify $KEPLER_SH is in path if not skip this test
  skipIfKeplerNotInPath

  mkdir -p "$THE_TMP/empty/data"
  echo "hi" > "$THE_TMP/empty/data/1.png"
  echo "hi" > "$THE_TMP/empty/data/2.png"

  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -imagedataset "$THE_TMP/empty" -filter ha -CWS_outputdir $THE_TMP $WF
  ...
  [ "${lines[1]}" == "detailed.error.message=No image matching (ha) filter exists in $THE_TMP/empty/data" ]
